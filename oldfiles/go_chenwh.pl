...
my $time=time;
my %opts;
GetOptions(\%opts,"h","i:s@","mark:s@","o:s","p:s","f:s","c:s","level:n","note:s","proc:s","func:s","comp:s","liner");
if (!$opts{i} or !$opts{o}){
    print STDERR <<"    _EOT_";
...
my @color_set=("#99330C","#9B97C8","#A52A2A","#DEB887","#5F9EA0","#7FFF00");
...
&get_data_for_GO_plot(\%hCelComponent,\%hMolFunction,\%hBioProcess,\@aXTickLabel,\%hYData,\@colum_count,$isLinerMode);
...
&out($x_end,$rect_width[scalar(@nm_go_file)-1],$moveper[scalar (@nm_go_file)-1],$isLinerMode,$yStep,$yEnd,$yStart);
for(@aXTickLabel){
	print O "$_\n";
}
print O ":End\n";
...
sub functional_catergory_of_go{#receive seven parameters, 1=hash_ref_for_catergory_results, 2=hash_ref_of_nm_go, 3=error_messages, 4=ontology_file_handle, 5=total_num_of_unigenes, 6=file_serial_num, 7=go_levels(go_depth)
	...
	while (my ($unigene_id,$go)=each %{$hReceivedNmGo}){
		...
		foreach(keys %hL2GOforThisUnigene){
			next if (/obsolete/);
			next if (/cellular_component/i or /molecular_function/i or /biological_process/i);
...
sub get_data_for_GO_plot{#accept five parameters, 1,2,3=hash_ref_for_go_catergories, 4=xticklabel, 5=hash_ref_to_receive_Y_data, 6=array_ref_to_receive_num_of_subcatergory;
	my ($go_comp,$go_func,$go_proc,$xticklabel,$y_data,$colum_stat,$linerMode)=@_;
