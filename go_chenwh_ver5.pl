...
## -- last modified : March 10, 2015; added options check for -gh and -ghprintgenelist
...
=========================================================================
    USAGE: perl $0
      ...
      [optional]
        *** at least one of the following three should be specified ***
        -p out file for plot
        -os print resuls for statistics analysis, only valid for two input files
        -osflag, a flag indicates whether the two input files are indepedent, default is none
            [*** for example, if one GO is all and another is part of all, the -osflag should be false
                 if one GO is bi and another is uni, the -osflag should be true
            ***]
        --------------------
...
die "EXIT: at least one of the following three has to be available : \n\t-o | -p | -os\n" if(!exists $opts{os} and !exists $opts{o} and !exists $opts{p});
die "EXIT: -gh is required if -ghprintgenelist is specified; please try again!! \n\n" if( exists $opts{ghprintgenelist} and !exists $opts{gh} ); ## march 2015!!! 
...
die "EXIT: option -os is not valid if number of input files is not equal to 2!!\n" if($fileNumCount != 2 and exists $opts{os});
...
#### for statistics test
if($fileNumCount == 2 and defined $opts{os}){
    my $file1 = defined $mark[0] ? $mark[0] : 'File1';
    my $file2 = defined $mark[1] ? $mark[1] : 'File2';
    
    my $rawdata = 'raw_'.int(rand(10000));
    my @aTitles = qw(aspect golevel goterm file1Total file2Total file1InThisCat file2InThisCat);
    open OUTSTAT, ">$rawdata" or die;
    print OUTSTAT "#", join("\n#", @nm_go_file), "\n";
    print OUTSTAT join("\t", @aTitles),"\n";
    ...
    foreach my $top_level (sort keys %hGOResults){
        ...
        foreach my $subcat (@aSubcat){
            my @aCounts = ();
            for (my $j=0; $j<@nm_go_file; $j++){
                my $members = exists $hGOResults{$top_level}{$subcat}{$j} ? scalar keys %{$hGOResults{$top_level}{$subcat}{$j}} : 0;
                push @aCounts, $members;
            }
            print OUTSTAT join("\t", $top_level, $level, $subcat, @maxNumOfUnigenes, @aCounts),"\n";
        }
    }
    close OUTSTAT;
    
    my $rscript = 'tmp_'.int(rand(10000));
    my $output = 'output_'.int(rand(10000));
    open TEMP, ">$rscript" or die;
    print TEMP "
    data <- read.table(file = \"$rawdata\", header = TRUE, as.is = TRUE, sep = \"\\t\");
    data[, \"note\"] <- '--';
    pvalues <- c();
    for(i in 1:dim(data)[1]){
        thisdata <- as.numeric(data[i, 4:7]);
        
        n0 <- thisdata[1]; # black and white, total
        n0.go <- thisdata[3]; #white
        n1 <- thisdata[2]; # sample
        n1.go <- thisdata[4]; # go in sample
        
        if($osflag){
            n.sampled.myGo <- n1.go
            n.sampled.otherGo <- n1 - n1.go
            n.notSampled.myGo <- n0.go
            n.notSampled.otherGo <- n0 - n0.go
        }else{
            n.sampled.myGo <- n1.go
            n.sampled.otherGo <- n1 - n1.go
            n.notSampled.myGo <- n0.go - n1.go
            n.notSampled.otherGo <- (n0 - n1) - (n0.go - n1.go)
        }

        # contingency table:
        t <- matrix(data=c(n.sampled.myGo, n.sampled.otherGo, n.notSampled.myGo, n.notSampled.otherGo), nrow=2, ncol=2)
        colnames(t) <- c(\"sampled\", \"notSampled\")
        rownames(t) <- c(\"myGo\", \"otherGo\")
        
        ratio <- thisdata[4] / thisdata[3] - thisdata[2] / thisdata[1];
        if(ratio > 0){
            pval <- fisher.test(t, alternative=\"greater\")\$p.value
        }else{
            pval <- fisher.test(t, alternative=\"less\")\$p.value;
        }
        ...
        data[i,\"p_fisher\"] <- pval;
        pvalues[i] <- pval;
    }
    
    library(\"LBE\");
    fdr <- LBE(pvalues, plot.type = \"none\");
    data[,\"fdr\"] <- fdr\$qval;
    write.table(data, file = \"$output\", sep = \"\\t\", quote=FALSE, row.names = FALSE, col.names = FALSE); # no title
    \n";
    close TEMP;
    system("R < $rscript --slave --vanilla");

    my $outXlsFile = $opts{os}.'.xls';
...
sub printgohierarchy{
	...
	#print STDERR "\tflag_print is : " , $flag_print, "\n";
