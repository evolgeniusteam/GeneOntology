...
GetOptions(
    \%opts,       "g:s",     "o:s",     "i:s@",
    "mark:s@",    "level:n", "title:s", "gh:s",
    "ghlevel:n",  "liner",   "p:s",     "om",
    "ghcutoff:f", "os:s",    "osflag",  "ghprintgenelist"
);
...
die "\n\tERROR:The number of input file should not bigger than 8!"
  if ( @nm_go_file > 8 );
...
my %hYData = ()
  ; #a hash contains pointers to arrays, its data structure is: $hYData{file_num}=@array of frequencies of eash x-element
...
my %hYTick = ()
  ; #a hash contains pointers to arrays. like: $hYTick{file1}=[array of YTicks of file1]
for ( my $i = 0 ; $i < @nm_go_file ; $i++ ) {
    my $max_num = $maxNumOfUnigenes[$i];
    @{ $hYTick{$i} } =
      ( int( $max_num / 10000 ), int( $max_num / 100 ), $max_num );
}
...
sub gohierarchy {
    ...
    $this_level++;
    &gohierarchy( $this_level, $refArrary, $$refHash{$name}{subcat},
        $refGeneHash );
}
...
sub printgohierarchy {
    ...
    foreach my $key ( sort keys %{$refHash} ) {
        ...
        if ( $fileNumCount == 2 ) {
            my $memCountOne =
              ( exists $$refHash{$key}{memmber}{0} )
              ? scalar( keys %{ $$refHash{$key}{memmber}{0} } )
              : 0;
            my $memCountTwo =
              ( exists $$refHash{$key}{memmber}{1} )
              ? scalar( keys %{ $$refHash{$key}{memmber}{1} } )
              : 0;
            my ( $totalGeneCountOne, $totalGeneCountTwo ) = @maxNumOfUnigenes;

            # --> now I have all the data
            # fisher's exact test first
            my $fisher = calculateStatistic(
                n11 => $memCountOne,
                n1p => $totalGeneCountOne,
                np1 => $memCountOne + $memCountTwo,
                npp => $totalGeneCountOne + $totalGeneCountTwo
            );
            my $dhyper =
              hypergeom( $memCountOne, $totalGeneCountOne - $memCountOne,
                $totalGeneCountTwo, $memCountTwo )
              ;    # good balls, bad balls, polled balls, polled good balls
            my $tag = ( $memCountTwo / $totalGeneCountTwo >
                  $memCountOne / $totalGeneCountOne ) ? 'enriched' : 'depleted';
            $flag_print = 0 if ( $ghcutoff <= $dhyper );
            print GOH "\t" x $this_level, $key, "\t", $fisher, "\t", $dhyper,
              "\t", $tag, "\n"
              if ($flag_print);
        ...
        if ($flag_print) {
            for ( my $i = 0 ; $i < @nm_go_file ; $i++ ) {
                ...
                if ( exists $$refHash{$key}{memmber}{$i} ) {
                    $memmber_count =
                      scalar( keys %{ $$refHash{$key}{memmber}{$i} } );
                }
                print GOH "\t" x ( $this_level + 1 ), "\t", $file_name, "\t",
                  $memmber_count, "\t",
                  $memmber_count / $maxGenesCountInCurrentFile * 100;
                print GOH "\t",
                  join( ",", keys %{ $$refHash{$key}{memmber}{$i} } )
                  if ($isPrintGHGeneList);
                print GOH "\n";
            }
        }
